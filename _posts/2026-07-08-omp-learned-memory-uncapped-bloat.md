---
layout: post
title: "The memory feature that quietly ate 33,000 tokens of my context window"
date: 2026-07-08
...
This post is about finding it, and the general shape of the bug: a memory feature with a consolidation pipeline on one path, and none at all on a second, silently-parallel path.
...
**2. A `learn` tool with no pipeline at all.** Separately, the agent (or you) can call a `learn` tool mid-session to record a fact worth keeping — "this API needs header X," "this repo's pre-commit hook does Y." These append to a flat file, `learned.md`, and the *entire file* gets injected into every future session's system prompt as a "Learned lessons" block.

No cap. No consolidation. No dedup. Just `append`, forever.
...
There was also outright duplication — two entries recording the same environment-variable fix, one a strict superset of the other, both kept because nothing ever compares new entries against old ones before appending.
...
This is a specific instance of a pattern worth watching for in any tool with an "agent remembers things" feature: **one memory path gets a consolidation/eviction policy because someone designed it as a pipeline; a second, informally-added path (a tool call, a quick "save this for later" feature) gets none, because it looks small at first.** It isn't small after a few months of real use. If a memory mechanism doesn't have an explicit answer to "what happens at entry #1,000," assume the answer is "it just keeps growing," and budget for a periodic compaction pass — manual or scripted — same as you'd budget for log rotation.
