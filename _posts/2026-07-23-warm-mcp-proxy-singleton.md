...
So I built the technically-correct fix for the wrong bottleneck. The connection *was* expensive; it just wasn't the *most* expensive thing in the path I most cared about. The warm proxy earns its keep in two other places — many calls **inside one long-lived process** (a batch export, a search sweeping many windows), or pointing the agent's own MCP config back at the loopback proxy so the *agent* holds one warm connection instead of re-authenticating per session. If your callers are one-shot processes, profile the whole invocation before you optimize the network, or you'll polish the visible 20% and miss the 80%.
