...
> **Generated by Claude (Anthropic) based on a real debugging session with [@ankitg12](https://dev.to/ankitg12).** This post exists because it took ~102,000 tokens of back-and-forth to get Unix tools working in PowerShell ΓÇö every pitfall documented here was actually hit. The goal: save you those tokens. All code was tested and verified working on PowerShell Core 7.5.4 / Windows 11.
>
...
Get common Unix commands ΓÇö `ls -lt`, `grep`, `sed`, `awk`, `head`, `tail` ΓÇö working with real Unix-style flags in **PowerShell Core 7.x on Windows 11**.

---

## Don't Reinvent the Wheel ΓÇö Existing Tools First
...
We went with **Scoop** to install individual GNU tools ΓÇö no WSL required, small installs, real binaries:
...
The scoop package for `uutils-coreutils` installs a **single multicall binary** (`coreutils.exe`) ΓÇö similar to BusyBox. Check what's actually bundled:
...
**Rule:** If a scoop shim exists for the binary, don't add a function wrapper. Just fix PATH if needed. For tools only available via coreutils multicall (`head`, `tail`, `wc`, `du`, `df`), use functions ΓÇö but remove any conflicting aliases first:
...
Fix ΓÇö add scoop shims to the front of PATH in your profile, but make it idempotent so reloading profile doesn't keep prepending:
...
```powershell
# DON'T ΓÇö this spawns Windows PowerShell 5.x, not your pwsh 7.x session
...
```powershell
# DO ΓÇö run directly in your pwsh session
...
`ls` is a built-in PowerShell alias for `Get-ChildItem`. Since aliases beat functions, you must remove it before defining a custom `ls`. Also, `Format-Table` inside a function conflicts with PowerShell's outer formatter when the function output is piped ΓÇö use `Select-Object` to emit plain objects instead:
...
## Further Reading
- [uutils/coreutils](https://github.com/uutils/coreutils) ΓÇö the Rust GNU coreutils rewrite
- [PowerShell-WSL-Interop](https://github.com/mikebattista/PowerShell-WSL-Interop) ΓÇö if you have WSL, this gives you the real thing
- [unix-pwsh](https://github.com/CrazyWolf13/unix-pwsh) ΓÇö a ready-made profile alternative
- [Scoop](https://scoop.sh) ΓÇö the package manager that makes all of this possible
